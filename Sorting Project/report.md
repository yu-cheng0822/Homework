...
### 1. 插入排序 (Insertion Sort) — $O(n)$ 與 $O(n^2)$ 的對比
...
### 2. 快速排序 (Quick Sort) — 三數中位數 (Median-of-Three) 
...
### 3. 合併排序 (Merge Sort) — 迭代版的穩定性

- **高度穩定性**：本實作採用非遞迴的**迭代版合併排序**，透過雙層迴圈（步長 `w` 每次乘以 2）從小區間一路合併至整個陣列。由於不論初始資料分布為何，演算法都必須嚴格執行完整的合併流程，因此在 Best、Avg、Worst 三種情況下的耗時幾乎完全一致，展現出高度穩定的 $O(n \log n)$ 特性。
...
### 4. 堆疊排序 (Heap Sort) — 手寫二元堆積

- **常數小且高度穩定**：堆疊排序在 Best、Avg、Worst 狀況下均穩定展現出 $O(n \log n)$ 的高效率，數據波動極小。
- 本程式**完全不依賴 STL 的 `make_heap` 與 `sort_heap` **，而是手寫實現 `heapify`（向下調整）與 `Heap` 排序主邏輯。透過將陣列以 0-based 索引視為完全二元樹（左小孩為 `2*i+1`，右小孩為 `2*i+2`），先由底向上建立最大堆積（Max Heap），再依次將樹根與末尾元素交換並調整。
- **空間與常數開銷**：排序過程完全在原陣列空間內進行（In-place），不需要任何額外的動態記憶體配置。這份完全由底層手寫的演算法，在免除記憶體開銷的同時，拿到了非常漂亮的漸進時間常數。
...
本實驗在 Visual Studio 與 Online C++ 雙平台進行測試，雖然兩者的 Big-O 漸進趨勢完全一致，但絕對執行時間存在些微差異：

- 編譯器優化：在 Debug 模式下包含較多安全檢查，而 Online C++ 通常預設開啟最佳化。在開啟優化後，迴圈展開與暫存器配置會顯著壓低 O(n \log n) 演算法的常數時間。
- 硬體快取：當 n <= 5000 時，資料量能完全容納在 CPU 的 L1/L2 快取中。這使得具備連續記憶體存取優勢的快速排序與堆疊排序，在實測中展現出極佳的常數表現。

#### 2. 計時精度與工程誤差控制

由於資料量較小時，單次排序時間常低於系統時鐘解析度（導致顯示為 0.0000 秒），我們採取了以下工程對策來將誤差控制在 1% 以內：

- 重複實驗取平均：採用 RUNS = 10 並在每次執行前重新洗牌，透過算術平均數有效平滑了背景行程切換的計時誤差。
- 隨機化抗干擾：利用演算法生成隨機測資，消除了特定數列對極端情況的影響。

#### 3. 演算法實務選擇與防禦性

- 防禦性設計：傳統快排在處理已排序或逆序資料時，容易退化至 O(n^2) 甚至引發 Stack Overflow。本程式實作「三數中位數」取樞紐，成功將最壞情況的機率壓至趨近於零，穩定在O(n \log n) 效能。
- 空間與穩定性的權衡：合併排序雖然穩定，但迭代版仍需 O(n) 的額外空間；而二元堆積的堆疊排序，完美實現了 In-place（原地級）排序，額外空間僅需 O(1)，是記憶體受限場景下的最佳選擇。
